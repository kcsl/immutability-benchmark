...
public class tsResultSet
{
   ...
   * the rowcache...
   */
  private  /*@RepRep*/  Vector rows; // all the rows - Contains ResultRows, which Contain DataRows
  ...
  /** State information */
  private  /*@OwnOwn*/  tinySQL dbengine;
  ...
  private  /*@NoRep*/  tsResultRow resultRowPrototype;
                     ...
                     tinySQL dbeng)
  {
    ...
    tables = new Vector();
     /*@OwnOwn*/  Enumeration tableEnum = tableColumns.keys();
  ...
  private boolean getMoreResults(int newPos, int fetchsize) throws tinySQLException
  {
    ...
    if (dbengine != null)
    {
      try
      {
        ...
        level = dbengine.continueQuery(( /*@NoRep*/  tsResultSet)this);
        if (level > 0)
        {
          eof = false;
          return eof;
  ...
  {
    hasReadResults = true;
    tsResultRow addrsRow = new  /*@NoRep*/  tsResultRow(resultRowPrototype);
  ...
  public void close()
  {
    ...
    Enumeration enumNew = tables.elements();
    while (enumNew.hasMoreElements())
    {
      tinySQLTableView table = (tinySQLTableView) enumNew.nextElement();
      try
      {
        table.close();
      }
